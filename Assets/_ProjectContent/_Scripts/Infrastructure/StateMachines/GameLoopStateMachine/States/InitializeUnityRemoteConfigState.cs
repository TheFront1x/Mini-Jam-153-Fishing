...
{
    public class InitializeUnityRemoteConfigState : BaseGameLoopState, IEnterableState
    {
        private readonly IConditionalLoggingService _conditionalLoggingService;

        private bool _isInitialized;

        public override string StateName => nameof(InitializeUnityRemoteConfigState);

        [Inject]
        public InitializeUnityRemoteConfigState(
        ...
        private async UniTask InitializeRemoteConfigAsync()
        {
            ...
            _conditionalLoggingService.Log("Start of InitializeRemoteConfigAsync with environment: production", LogTag.RemoteSettings);
#else
            var options = new InitializationOptions().SetEnvironmentName("production");
#endif
